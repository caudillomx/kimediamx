...
import { ArrowLeft, Building2, Lock, CheckCircle, BookOpen } from "lucide-react";
import { Link } from "react-router-dom";
import { Button } from "@/components/ui/button";
import { GuideRegistrationModal } from "@/components/guide/GuideRegistrationModal";
import { pymeGuide, GuideChapter } from "@/data/guideContent";
import { supabase } from "@/integrations/supabase/client";
import { toast } from "sonner";

export default function GuidePyme() {
  ...
  useEffect(() => {
    const storedEmail = localStorage.getItem("kimedia_guide_pyme");
    if (storedEmail) {
  ...
  const handleRegister = async (data: { name: string; email: string; phone?: string; companyName?: string }) => {
    ...
    try {
      ...
      localStorage.setItem("kimedia_guide_pyme", data.email);
      setIsRegistered(true);
      ...
      <header className="fixed top-0 left-0 right-0 z-40 bg-background/80 backdrop-blur-lg border-b border-border">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-foreground hidden sm:inline">Guía PyME</span>
          </div>
        </div>
      </header>
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-cyan/10 border border-cyan/20 mb-6">
              ...
              <span className="text-sm font-medium text-cyan">Guía completa</span>
            </div>
                  ...
                  <div
                    ...
                    className={`p-6 rounded-xl border cursor-pointer transition-all ${
                      isActive
                        ...
                        : "bg-card border-border hover:border-cyan/50"
                    }`}
                  >
      ...
      {!isRegistered && (
        <section className="py-16 px-4">
          <div className="container mx-auto max-w-2xl text-center">
            <div className="p-8 rounded-2xl bg-gradient-to-br from-cyan/10 to-electric/10 border border-cyan/20">
              <h2 className="text-2xl font-bold text-foreground mb-4">
                Desbloquea los 10 capítulos
              </h2>
              <p className="text-muted-foreground mb-6">
                Accede a todo el contenido premium, plantillas y el plan de implementación de 30 días.
              </p>
              <Button
                onClick={() => setIsModalOpen(true)}
                className="bg-gradient-to-r from-cyan to-electric hover:opacity-90 text-white font-semibold py-6 px-8"
