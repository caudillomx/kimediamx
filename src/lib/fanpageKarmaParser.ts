...
export async function parseComparativa(file: File): Promise<ParsedFile<ComparativaRow>> {
  ...
  for (const raw of dataRows) {
    ...
    rows.push({
      ...
      performanceIndex: toNum(r["Índice de Rendimiento de la página"] ?? r["Índice de Rendimiento"] ?? r["Performance Index"]),
      followers: toNum(r["Seguidor"] ?? r["Seguidores"] ?? r["Followers"]),
      followerGrowthRate: toNum(r["Crecimiento de seguidores por día en %"] ?? r["Crecimiento de seguidores"]),
