...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: data.error || `Error ${response.status}` }),
    ...
    return new Response(
      JSON.stringify({ success: false, error: error instanceof Error ? error.message : 'Error al extraer contenido' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  }
});
